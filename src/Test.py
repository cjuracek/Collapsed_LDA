...
if __name__ == "__main__":
    ...
    test_list2 = ["a", "b", "c", "d", "w", "i"]
    matrix1 = np.array(
        [[0, 0, 0, 0.05, 1, 0.95], [1, 0.95, 0, 0, 0, 0.05], [0, 0.05, 1, 0.95, 0, 0]]
    )
    ...
    assert get_top_n_words(matrix1, 2, test_list2) == {
        1: ["w", "i"],
        2: ["a", "b"],
        3: ["c", "d"],
    }, "Does not return the right words"

    # Checking 'tokenize_doc'
    string_test = "We EAT on the coffee-table! U.S.A. '56'"
    assert tokenize_doc(string_test) == [
        "we",
        "eat",
        "on",
        "the",
        ...
        "usa",
        "56",
    ], "Sould return ['we', 'eat', 'on', 'the', 'coffeetable', 'usa', '56']"
    ...
    tokens = remove_stop_words(
        tokens, remove_numbers=True, tokens_have_quotes=False, extra_words=extra_words
    )
